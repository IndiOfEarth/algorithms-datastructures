// LOTTERY PROBLEM ASSIGNMENT
// Utilising Linear Search, Bubble Sort
...
function find(birthdays) {
    ...
    // * Compares each value with other values in the array
    // * If values are unique they are added to the array
    for (var i = 1; i < n; i+=2) {
        count = 0; // resets count for each iteration
        for (var j = 1; j < n; j+=2) {
            // check if ids are same - if they are then don't compare
            if (i != j) {
                // Compare the birthdays
        ...
        // Adds unique players to the array
        if (count == 0) {
            players.push(birthdays[i-1]); // adds the id for the player
        }
    }
    return players;
}
...
function findSorted(birthdays) {
	// find the unique birthdays by using bubble sort
    // then use linear search
    var players = find(bubbleSortDays(bubbleSort(birthdays)));
    return players;
